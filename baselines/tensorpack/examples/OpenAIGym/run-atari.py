...
def get_player(dumpdir=None):
    pl1 = GymEnv(ENV_NAME, dumpdir=dumpdir, auto_restart=False)
    pl = MapPlayerState(pl1, lambda img: cv2.resize(img, IMAGE_SIZE[::-1]))
...
def do_submit(output):
    gym.upload(output, api_key='sk_h8fTmecHReihR9ldmfwjftixrqFTkSZ124WFVV1A')
...
if __name__ == '__main__':
    ...
    p, _ = get_player(); del p    # set NUM_ACTIONS
